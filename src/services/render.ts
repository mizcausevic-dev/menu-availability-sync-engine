...
<head>
  ...
  <title>${title}</title>
  <meta name="description" content="TypeScript control plane for menu availability, sync blockers, channel posture, and buyer-safe restaurant commerce operations. Operator surface in the Kinetic Gain Suite." />
  ...
  <link rel="canonical" href="https://menus.kineticgain.com/" />
  <link rel="icon" type="image/svg+xml" href="${KG_FAVICON_DATA_URI}" />
  <meta property="og:type" content="website" />
  <meta property="og:site_name" content="Kinetic Gain" />
  <meta property="og:title" content="${title}" />
  <meta property="og:url" content="https://menus.kineticgain.com/" />
  <meta property="og:description" content="TypeScript control plane for menu availability, sync blockers, channel posture, and buyer-safe restaurant commerce operations. Operator surface in the Kinetic Gain Suite." />
  <meta name="twitter:card" content="summary" />
  <meta name="twitter:title" content="${title}" />
  <meta name="twitter:description" content="TypeScript control plane for menu availability, sync blockers, channel posture, and buyer-safe restaurant commerce operations. Operator surface in the Kinetic Gain Suite." />
  <script type="application/ld+json">{"@context":"https://schema.org","@graph":[{"@type":"ProfessionalService","@id":"https://menus.kineticgain.com/#service","name":"Kinetic Gain — TypeScript control plane for menu availability, sync blockers, channel posture, and buyer-safe restaurant commerce operations","url":"https://menus.kineticgain.com/","areaServed":"United States","description":"TypeScript control plane for menu availability, sync blockers, channel posture, and buyer-safe restaurant commerce operations. Operator surface in the Kinetic Gain Suite."},{"@type":"WebSite","@id":"https://menus.kineticgain.com/#website","url":"https://menus.kineticgain.com/","name":"TypeScript control plane for menu availability, sync blockers, channel posture, and buyer-safe restaurant commerce operations","publisher":{"@type":"Organization","name":"Kinetic Gain LLC"}}]}</script>
  <style>${KG_STYLE01_CSS}${KG_COMPAT_CSS}${MENU_SURFACE_CSS}</style>
</head>
<body>
  <header>
    <div class="wrap nav">
      <a class="kg-logo" href="/" aria-label="Kinetic Gain — TypeScript control plane for menu availability, sync blockers, channel posture, and buyer-safe restaurant commerce operations">
        ${KG_MARK_SVG}
        <span class="kg-word">Kinetic Gain</span>
      </a>
      <nav class="nav-links" id="primaryNav">
        <a href="/availability-lane">Availability Lane</a>
        <a href="/sync-risks">Sync Risks</a>
        <a href="/channel-posture">Channel Posture</a>
    ...
    <div class="wrap">
      <div class="foot-top">
        <div>
          ...
          <p class="foot-tag">TypeScript control plane for menu availability, sync blockers, channel posture, and buyer-safe restaurant commerce operations — operator surface in the Kinetic Gain Suite. Static demo data; no live calls.</p>
        </div>
        <div class="foot-cols">
          <div class="foot-col">
            <h4>Estate</h4>
          ...
          <div class="foot-col">
            <h4>This surface</h4>
            <a href="https://github.com/mizcausevic-dev/menu-availability-sync-engine">GitHub repo</a>
          </div>
        </div>
      </div>
      <div class="foot-bot">
...
function topbar(active: string) {
  ...
  const links = [
    ...
    {
      "href": "/availability-lane",
      "label": "Availability Lane"
    },
    {
      "href": "/sync-risks",
      "label": "Sync Risks"
    ...
    {
      "href": "/channel-posture",
      "label": "Channel Posture"
    },
    {
      "href": "/verification",
      "label": "Verification"
...
function renderHero(active: string) {
  ...
  const urgentCase = availabilityLane().find((item) => item.risk === "red") ?? availabilityLane()[0];
  const syncRisk = syncRisksLane().find((item) => item.readiness === "red") ?? syncRisksLane()[0];
  const packet = channelPosture().find((item) => item.status !== "green") ?? channelPosture()[0];
  return `${topbar(active)}
  <section class="hero reveal">
    <div class="acard hero-panel">
      <div class="hero-grid">
        <div class="hero-copy">
          <span class="eyebrow">Menu availability</span>
          <div class="hero-meta">
          ...
          <h1>Where do menu changes quietly become marketplace and customer-trust risk?</h1>
          <p class="hero-subtle">Menu Availability Sync Engine keeps availability parity, modifier drift, cache staleness, and channel-safe launch posture visible before the next promo or discovery window turns a healthy menu update into customer-facing breakage.</p>
          <div class="hero-nav">${routePills(active, links)}</div>
          <div class="hero-kpis">
            <div class="stat"><label>Cases</label><strong>${metrics.cases}</strong><span>Active menu-sync cases tied to channel, owner, and next move.</span></div>
            <div class="stat"><label>Urgent cases</label><strong>${metrics.urgentCases}</strong><span>Red cases where customer-facing availability or pricing is already under pressure.</span></div>
            <div class="stat"><label>Blocked syncs</label><strong>${metrics.blockedSyncs}</strong><span>Evidence gaps still waiting on parity, pricing, or cache proof.</span></div>
            <div class="stat"><label>Fragile packets</label><strong>${metrics.fragilePackets}</strong><span>Channel-facing packets still too weak to publish safely.</span></div>
          </div>
        </div>
        <aside class="hero-aside">
          <div class="acard">
            <span class="metric-chip">Current watch item</span>
            <h3>${urgentCase.brand} · ${urgentCase.item}</h3>
            <p><strong>${urgentCase.channel}</strong><br />${urgentCase.excerpt}</p>
          </div>
          <div class="acard">
            ...
            <ul class="hero-mini-list">
              ...
              <li><strong>${syncRisk.owner}</strong><span>${syncRisk.requiredEvidence}</span></li>
              <li><strong>${packet.audience}</strong><span>${packet.decisionNote}</span></li>
            </ul>
          </div>
          <div class="acard">
            <span class="metric-chip">Control surfaces</span>
            <p>POS events, marketplace webhooks, pricing bundles, cache freshness, and channel-safe packet posture stay in one readable view instead of drifting across commerce, growth, and store operations.</p>
          </div>
        </aside>
      </div>
    </div>
  ...
  return layout(
    ...
    heroShell(
      ...
      `<section class="section">
        <div class="sh"><h2>Availability lane</h2><div class="note">Channel, pricing, and cache pressure that leadership can actually act on</div></div>
        <div class="section-cards">
          ${cases.map((item) => `<article class="acard surface-card"><span class="tag ${item.risk}">${item.risk}</span><h3>${item.brand} · ${item.item}</h3><div class="surface-meta"><p><strong>${item.caseId}</strong> · ${item.region} · ${item.channel}</p><p><strong>Owner:</strong> ${item.owner}</p></div><p class="surface-copy">${item.excerpt}</p><p class="surface-next"><strong>Next move:</strong> ${item.nextAction}</p></article>`).join("")}
        </div>
      </section>
      <section class="section">
        <div class="sh"><h2>Sync pressure</h2><div class="note">Where parity, pricing, and cache freshness can still break customer trust</div></div>
